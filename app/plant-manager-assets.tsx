...
export default function PlantManagerAssetsScreen() {
  ...
  const loadAssets = useCallback(async () => {
    ...
    if (!user?.masterAccountId || !user?.siteId) {
      console.log('[PlantManagerAssets] ERROR: No masterAccountId or siteId');
      console.log('[PlantManagerAssets] user:', JSON.stringify(user));
      setIsLoading(false);
      return;
    }
    ...
    try {
      setIsLoading(true);
      console.log('[PlantManagerAssets] Current user details:');
      console.log('[PlantManagerAssets]   - userId:', user.userId);
      console.log('[PlantManagerAssets]   - role:', user.role);
      console.log('[PlantManagerAssets]   - masterAccountId:', user.masterAccountId);
      console.log('[PlantManagerAssets]   - siteId:', user.siteId);
      console.log('[PlantManagerAssets]   - siteName:', user.siteName);
      console.log('[PlantManagerAssets]   - companyId:', user.currentCompanyId);
      ...
      console.log('[PlantManagerAssets] Executing Firebase query...');
      const querySnapshot = await getDocs(q);
      console.log('[PlantManagerAssets] Query returned', querySnapshot.size, 'total documents');
      
      const loadedAssets: PlantAsset[] = [];
      
      querySnapshot.forEach((doc) => {
        const data = doc.data() as PlantAsset;
        console.log('[PlantManagerAssets] Raw document:', doc.id, '- Data:', {
      ...
      console.log('[PlantManagerAssets] Total loaded before filtering:', loadedAssets.length);
      const activeAssets = loadedAssets.filter((a) => !a.archived);
      console.log('[PlantManagerAssets] Active assets (non-archived):', activeAssets.length);
      
      activeAssets.forEach((asset, index) => {
        console.log(`[PlantManagerAssets] Asset ${index + 1}:`);
        console.log(`  - ID: ${asset.id}`);
        console.log(`  - AssetId: ${asset.assetId}`);
        console.log(`  - Type: ${asset.type}`);
        console.log(`  - Location: ${asset.location}`);
        console.log(`  - SiteId: ${asset.siteId}`);
        console.log(`  - MasterAccountId: ${asset.masterAccountId}`);
        console.log(`  - Archived: ${asset.archived}`);
      });
