...
import { FileText, Calendar, DollarSign, Users, CheckCircle, AlertCircle, Clock, MessageSquare, X, Send } from 'lucide-react-native';
import { Colors } from '@/constants/colors';
import { useAuth } from '@/contexts/AuthContext';
import { getEMHReportsForRecipient, approveEMHReport, disputeEMHReport } from '@/utils/emhReportManager';
import { EMHReport, LineItemDispute } from '@/types/emhReport';
...
export default function EMHInboxScreen() {
  ...
  const loadEMHReports = useCallback(async () => {
    ...
    try {
      setLoading(true);
      console.log('[EMH Inbox] Loading reports for:', user.masterAccountId);
      const fetchedReports = await getEMHReportsForRecipient(user.masterAccountId);
      setReports(fetchedReports);
      console.log('[EMH Inbox] Loaded', fetchedReports.length, 'reports');
    } catch (error) {
  ...
  const handleApprove = async (report: EMHReport) => {
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await approveEMHReport(report.id!, user.userId);
              Alert.alert('Success', 'EMH report approved');
              loadEMHReports();
            } catch (error) {
              console.error('[EMH Inbox] Error approving:', error);
  ...
  const handleDispute = async (report: EMHReport) => {
    ...
    try {
      ...
      if (report.lineItemDisputes && report.lineItemDisputes.length > 0) {
        ...
        report.lineItemDisputes.forEach(dispute => {
          const key = `${dispute.employeeId}-${dispute.date}`;
  ...
  const handleLineItemDispute = (timesheet: any) => {
    ...
    const key = `${timesheet.employeeId}-${timesheet.date}`;
  ...
  const submitLineItemDispute = () => {
    ...
    const key = `${selectedTimesheet.employeeId}-${selectedTimesheet.date}`;
  ...
  const removeLineItemDispute = (employeeId: string, date: string) => {
  ...
  const submitDispute = async () => {
    ...
    try {
      ...
      console.error('[EMH Inbox] Error disputing:', error);
  ...
  return (
    <SafeAreaView style={styles.container} edges={['bottom']}>
          ...
          <ScrollView style={styles.content}>
            {filteredReports.length === 0 ? (
              <View style={styles.emptyContainer}>
                ...
                <Text style={styles.emptyText}>No EMH reports</Text>
                <Text style={styles.emptySubtext}>
                  EMH reports from enterprise clients will appear here
                </Text>
              </View>
            ) : (
                            ...
                            {hasDispute && dispute && (
                              <View style={styles.lineDisputeBox}>
                                ...
                                <TouchableOpacity
                                  ...
                                  onPress={() => removeLineItemDispute(timesheet.employeeId, timesheet.date)}
                                >
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <Text style={styles.modalSubtitle}>
              {selectedTimesheet.employeeName} - {selectedTimesheet.employeeRole}
            </Text>
            ...
            <View style={styles.disputeInfoBox}>
              ...
              <Text style={styles.disputeInfoLabel}>Regular: {selectedTimesheet.regularHours}h | Overtime: {selectedTimesheet.overtimeHours}h</Text>
            </View>
            <Text style={styles.modalLabel}>What&apos;s wrong with this entry?</Text>
            <TextInput
              style={styles.modalInput}
              placeholder="E.g., Hours should be 7.5h not 8h, employee was absent after lunch..."
      ...
      {disputeModalVisible && selectedReport && (
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>Dispute EMH Report</Text>
            <Text style={styles.modalSubtitle}>
            ...
            <TextInput
              style={styles.modalInput}
              placeholder={lineItemDisputes.size > 0 ? "Add general comments..." : "E.g., Hours don't match our records for Employee John Doe..."}
              placeholderTextColor={Colors.textSecondary}
              value={disputeNotes}
