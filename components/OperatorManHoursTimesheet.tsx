...
export default function OperatorManHoursTimesheet({
  ...
  const checkTimeLock = useCallback(() => {
    const now = new Date();
    ...
    return timeInMinutes >= lockTimeInMinutes;
  }, []);

  const loadExistingTimesheet = useCallback(async () => {
    try {
      setIsLoadingTimesheet(true);
      const today = new Date().toISOString().split('T')[0];
      
      console.log('[OperatorManHours] Loading timesheet for date:', today, 'operatorId:', operatorId);
      ...
      if (!snapshot.empty) {
        const timesheetDoc = snapshot.docs[0];
        const data = timesheetDoc.data();
        
        console.log('[OperatorManHours] Existing timesheet found:', data);
        
        setExistingTimesheetId(timesheetDoc.id);
        setStartTime(data.startTime || '');
        setStopTime(data.stopTime || '');
        setNoLunchBreak(data.noLunchBreak || false);
        setNotes(data.notes || '');
        setStartTimeCommitted(!!data.startTime);
        
        const locked = checkTimeLock();
        setIsLocked(locked && !isAdminOrHR);
        
        console.log('[OperatorManHours] Timesheet loaded - locked:', locked, 'isAdminOrHR:', isAdminOrHR);
      } else {
        console.log('[OperatorManHours] No existing timesheet for today');
        setIsLocked(false);
      }
    } catch (error) {
      console.error('[OperatorManHours] Error loading timesheet:', error);
    } finally {
      setIsLoadingTimesheet(false);
    }
  }, [operatorId, checkTimeLock, isAdminOrHR]);
  ...
  const handleSubmitTimesheet = async () => {
    if (isLocked && !isAdminOrHR) {
      Alert.alert('Locked', 'This timesheet is locked. Only Admin or HR can make changes.');
      return;
    }
    ...
    try {
      ...
      const timesheetData = {
        ...
        editedBy: isAdminOrHR ? userRole : null,
      };
      ...
      if (netInfo.isConnected) {
        ...
        if (existingTimesheetId) {
          ...
          await updateDoc(doc(db, 'operatorTimesheets', existingTimesheetId), {
            ...timesheetData,
            updatedAt: serverTimestamp()
          });
          console.log('[OperatorManHoursTimesheet] Timesheet updated online');
        } else {
          // Create new timesheet
          const docRef = await addDoc(collection(db, 'operatorTimesheets'), {
            ...timesheetData,
            createdAt: serverTimestamp(),
            updatedAt: serverTimestamp()
          });
          setExistingTimesheetId(docRef.id);
          console.log('[OperatorManHoursTimesheet] Timesheet created online');
        }
        
        if (locked) {
          setIsLocked(!isAdminOrHR);
        }
      } else {
        ...
        if (existingTimesheetId) {
          ...
          console.log('[OperatorManHoursTimesheet] Timesheet update queued offline');
        } else {
          await queueFirestoreOperation(
            {
              type: 'add',
      ...
      const action = existingTimesheetId ? 'updated' : 'submitted';
      
      let summaryMessage = `Man hours ${isOffline ? 'saved for sync' : action + ' successfully'}\n` +
        `Start: ${startTime}\n` +
        `Stop: ${stopTime}\n` +
      ...
      if (locked) {
        summaryMessage += '\n\n🔒 Locked at 23:55 - Only Admin/HR can edit';
      } else {
        summaryMessage += '\n\n✓ Editable until 23:55 today';
      }
      ...
      {isLocked && !isAdminOrHR && (
        <View style={styles.lockBanner}>
          ...
          <View style={styles.lockBannerText}>
            <Text style={styles.lockBannerTitle}>🔒 Timesheet Locked</Text>
            <Text style={styles.lockBannerMessage}>
              Locked at 23:55. Contact Admin or HR for changes.
            </Text>
          </View>
        </View>
      )}
      
      {isAdminOrHR && isLocked && (
        <View style={styles.adminBanner}>
          <AlertCircle size={20} color="#0369a1" />
          <View style={styles.adminBannerText}>
            <Text style={styles.adminBannerTitle}>🔓 Admin/HR Access</Text>
            <Text style={styles.adminBannerMessage}>
              You can edit this locked timesheet
            </Text>
          </View>
        </View>
      )}
      ...
      <View style={styles.formCard}>
        ...
        <View style={styles.timeSection}>
          ...
          <View style={styles.inputGroup}>
            ...
            <TextInput
              style={[styles.input, startTimeCommitted && styles.inputCommitted, (isLocked && !isAdminOrHR) && styles.inputDisabled]}
              ...
              maxLength={5}
              editable={!isSubmitting && !startTimeCommitted && (!isLocked || isAdminOrHR)}
            />
            {startTimeCommitted ? (
          ...
          <View style={styles.inputGroup}>
            ...
            <TextInput
              style={[styles.input, !startTimeCommitted && styles.inputDisabled, (isLocked && !isAdminOrHR) && styles.inputDisabled]}
          ...
          <View style={styles.lunchBreakRow}>
            ...
            <Switch
              ...
              disabled={isSubmitting || (isLocked && !isAdminOrHR)}
            />
          </View>
        ...
        <View style={styles.inputGroup}>
          ...
          <TextInput
            style={[styles.input, styles.textArea, (isLocked && !isAdminOrHR) && styles.inputDisabled]}
            ...
            editable={!isSubmitting && (!isLocked || isAdminOrHR)}
          />
        </View>

        {/* Submit Button */}
        <TouchableOpacity
          style={[styles.submitButton, (isSubmitting || (isLocked && !isAdminOrHR)) && styles.submitButtonDisabled]}
          ...
          {isSubmitting ? (
            ...
            <>
              ...
              <Text style={styles.submitButtonText}>
                {existingTimesheetId ? 'Update Man Hours' : 'Submit Man Hours'}
              </Text>
            </>
          )}
        </TouchableOpacity>
        ...
        <View style={styles.infoBox}>
          ...
          <Text style={[styles.infoText, { fontWeight: '700', marginTop: 8 }]}>
            🔒 Man hours can be logged once per day. Edits allowed until 23:55, then locked.
          </Text>
          {!isLocked && (
            <Text style={styles.infoText}>
              ✓ Currently editable until 23:55 today
            </Text>
          )}
        </View>
      </View>
