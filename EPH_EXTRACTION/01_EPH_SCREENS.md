# EPH Screens - Complete Code

This document contains all EPH-related screen files with full implementation code.

## Navigation Routes

- **EPH Inbox**: `/eph-inbox` - Main EPH reports inbox showing sent reports
- **EPH Menu**: `/eph-menu` - EPH menu placeholder screen

---

## 1. EPH Inbox Screen

**File Path**: `app/eph-inbox.tsx`

**Purpose**: Main inbox screen for viewing sent EPH reports with filtering, status tracking, and detailed timesheet views.

**Dependencies**:
- `@/utils/ephReportManager` - `getEPHReportsForSender()`
- `@/types/ephReport` - `EPHReport` type
- `@/contexts/AuthContext` - User authentication
- Firebase Firestore - `plantAssetTimesheets` collection

**Key Features**:
- Filter reports by status (all, sent, reviewed, agreed, disputed)
- View report summaries with costs and hours
- Drill down into detailed timesheets per report
- Three tabs: EPH Reports, EPH Approvals, EPH Payments
...
**File Path**: `app/eph-menu.tsx`

**Purpose**: Placeholder screen for EPH menu (future expansion).
...
## Firestore Queries Used by Screens

### EPH Inbox Screen Queries

1. **Load EPH Reports** (via `getEPHReportsForSender()`)
   - Collection: `ephReports`
   - Filters: `senderMasterAccountId == user.masterAccountId`

2. **Load Timesheet Details**
   - Collection: `plantAssetTimesheets`
   - Filters: 
     - `assetId == report.assetIds[i]`
     - `date >= report.dateRangeFrom`
     - `date <= report.dateRangeTo`
   - Order: `date DESC`
...
Continue to next batch for utility functions.
